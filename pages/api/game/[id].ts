...
import { connectDB, getAsync, setAsync } from "../../../lib/redis";
...
export default function handler(
  ...
  let action;
  if (req.body != "") action = JSON.parse(req.body) as Action;
}

function createGame(id: string, creatorID: string) {
  const game: Game = {
    id: id,
    creator: { id: creatorID, points: 0 },
...
export function getGames() {
  ...
  for (const game of open_games) {
    if (game.status === Status.PENDING) pending.push(game.id);
  }
  return pending;
}
